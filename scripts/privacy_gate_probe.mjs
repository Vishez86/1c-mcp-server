...
async function discover() {
  ...
  if (!denyAliases.length && !denyMasks.length) {
    // Политика без deny-типов (например ZUP_CORP, где значимы person_aliases):
    // матрица утечек §6 неприменима — закрывать нечего. Но правки R-01…R-11
    // меняют разбор ЛЮБОГО запроса, а разбор карты источников используется и
    // pre-flight проверками вне privacy. Поэтому здесь гоняется набор форм,
    // который обязан проходить независимо от политики (§7, второй контур ТЗ).
    record("политика", "типы с mode: deny", "SKIP",
      "политика без deny-типов: §6 неприменима, гоняем только формы запросов (§7)");
...
async function section6() {
  ...
  if (fixtures.accountingReady) {
    ...
    // R-04 вариант 2: переименованная колонка ВТ. До вывода типов из проекции
    // разрешение шло по имени поля и одного КАК хватало для обхода (§0.1 п.3).
    await mustDeny(S, "P5 ПРЕДСТАВЛЕНИЕ(Т.Аналитика) — переименованная колонка ВТ",
      `ВЫБРАТЬ О.Субконто2 КАК Аналитика ПОМЕСТИТЬ ВТ_П ИЗ ${ost}`
      + `\n;\nВЫБРАТЬ ПЕРВЫЕ 3 ПРЕДСТАВЛЕНИЕ(Т.Аналитика) КАК Н ИЗ ВТ_П КАК Т`, par);
    await mustDeny(S, "P6 Т.Аналитика.Наименование — переименованная колонка ВТ",
      `ВЫБРАТЬ О.Субконто2 КАК Аналитика ПОМЕСТИТЬ ВТ_П ИЗ ${ost}`
      + `\n;\nВЫБРАТЬ ПЕРВЫЕ 3 Т.Аналитика.Наименование КАК Н ИЗ ВТ_П КАК Т`, par);
  ...
  // з1/з2 в базовой линии уже отклонялись — держим как регресс-контроль.
  await mustDeny(S, "з1 ЗНАЧЕНИЕ(...ПустаяСсылка).Наименование",
    `ВЫБРАТЬ ЗНАЧЕНИЕ(${closed}.ПустаяСсылка).Наименование КАК Н`);
  await mustDeny(S, "з2 ЕСТЬNULL(К.Ссылка, ЗНАЧЕНИЕ(...)).Наименование",
    `ВЫБРАТЬ ПЕРВЫЕ 3 ЕСТЬNULL(К.Ссылка, ЗНАЧЕНИЕ(${closed}.ПустаяСсылка)).Наименование КАК Н ИЗ ${closed} КАК К`);
...
async function section7() {
  ...
  if (tabularName) {
    ...
    // настоящими наименованиями («Автотрейд», «ООО ""Автотрейд""»).
    await mustDeny("§6 обязан отказать", `т4 К.${tabularName}.Ссылка.Наименование`,
      `ВЫБРАТЬ ПЕРВЫЕ 5 К.${tabularName}.Ссылка.Наименование КАК Н ИЗ ${closed} КАК К`);
    await mustDeny("§6 обязан отказать", `т6 К.${tabularName}.Ссылка.НаименованиеПолное`,
      `ВЫБРАТЬ ПЕРВЫЕ 5 К.${tabularName}.Ссылка.НаименованиеПолное КАК Н ИЗ ${closed} КАК К`);
    await mustDeny("§6 обязан отказать", `т5 Т.Ссылка.Наименование из ${tabularName} как источника`,
