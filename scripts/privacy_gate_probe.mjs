...
async function sectionMask() {
  ...
  if (fixtures.openCatalog) {
    ...
    // О9 — контроль к M-02.6: у ВЫБРАТЬ * один элемент проекции и много колонок,
    // позиционное сопоставление применяться не должно.
    await mustNotMask(O, "О9 ВЫБРАТЬ * из открытого справочника", "Наименование",
