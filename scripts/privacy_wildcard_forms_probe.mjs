...
async function probeContour() {
  ...
  // ---- Ц: цена страховки. Колонка открытого типа, взятая через подзапрос,
  // вердикта первого эшелона больше не получает — решает второй эшелон по имени
  // ключа. Это ЗАМЕР, а не FAIL: цена названа в ТЗ (P-0 п. 1) и принята. Если
  // цена окажется велика, лечится разбором внутрь подзапроса (P-0 п. 4a,
  // дорогой вариант), а не возвратом вердикта по неосмотренной колонке.
  if (!fixtures.open) {
    record("Ц цена", "Ц1 открытый тип через подзапрос", "SKIP", "тип вне политики не найден");
  } else {
    const q = `ВЫБРАТЬ Т.${fixtures.openField} КАК Ф ИЗ (ВЫБРАТЬ О.${fixtures.openField}`
      + ` КАК ${fixtures.openField} ИЗ ${fixtures.open} КАК О) КАК Т`;
    const res = await runQuery(q);
    const rows = res.data?.rows ?? [];
    if (!res.ok || !rows.length) {
      record("Ц цена", "Ц1 открытый тип через подзапрос", "SKIP",
        `нет строк${res.transport ? `, транспорт: ${res.transport}` : ""}`);
    } else {
      const masked = rows.some((r) => hasValue(r.Ф) && looksMasked(r.Ф));
      record("Ц цена", "Ц1 открытый тип через подзапрос", "ЦЕНА",
        masked
          ? "значение ПОДМЕНЕНО вторым эшелоном: цена страховки видна, сверить с §3 ТЗ"
          : "значение открыто: страховка на этом ключе не срабатывает, цены нет");
    }
    const direct = await runQuery(
      `ВЫБРАТЬ ПЕРВЫЕ 3 О.${fixtures.openField} КАК Ф ИЗ ${fixtures.open} КАК О`);
    const rowsDirect = direct.data?.rows ?? [];
    check("Ц цена", "Ц2 контроль: прямая выборка открытого типа не подменена",
