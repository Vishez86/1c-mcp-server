...
// Имя объекта конфигурации автор запроса изменить не может (#145): слово,
// стоящее в пути метаданных сразу после вида объекта
// (Справочник.MCP_Маскирование), правилом не судится — иначе объект с латиницей
// в имени недостижим навсегда. Существование объекта офлайн-чекер проверить не
// может, это работа сервера (metadata_not_found); здесь снимается только
// приговор форме имени. Рукописные имена — временные таблицы, псевдонимы,
// поля через точку от псевдонима — судятся как прежде. Осознанная граница:
// вложенные сегменты пути (табличная часть, виртуальная таблица) под правилом
// остаются — их гомоглиф там так же недостижим для сервера, но случай не
// встречался, а ослаблять правило впрок нельзя.
const METADATA_KIND_BEFORE_DOT_RE = new RegExp(
  "(?:^|[^А-Яа-яЁёA-Za-z0-9_.])(?:Справочник|Документ|Перечисление|ПланСчетов" +
  ...
  "|ЖурналДокументов|КритерийОтбора|Константа)\\s*\\.\\s*$",
  "iu",
);
// Срез 160 символов: имя объекта метаданных не длиннее 128, и вид с точкой в
// срез попадает целиком — обрезанное посередине слово вид не имитирует.
const isMetadataObjectName = (code, index) =>
  METADATA_KIND_BEFORE_DOT_RE.test(code.slice(Math.max(0, index - 160), index));
...
function checkMixedScript(raw, code, findings) {
  ...
  while ((m = re.exec(code)) !== null) {
    ...
    if (isMetadataObjectName(code, m.index)) continue;
