...
const MODULE_MARKERS = [
  ...
  },
  {
    ...
    // since=824a3e8: снятие служебного ключа privacy_column_decisions на выходе
    // (#164). Признак проверяется третьим условием ниже — ключа в ответе нет.
    // since=4832b71: обе записи MCP.audit.failed переведены на метрики (стадия,
    // correlation_id, длина). Признака снаружи нет — это аварийная ветка самого
    // аудита; свежесть доказывает ревизия 2026-08-17.2, содержимое — ЖР изнутри 1С.
    // since=f31e9d6: схема get_audit_log объявляет event_names (#158). Признак —
    // by_event в ответе, проверяется маркером MCP_Audit «тайминги наружу» (кейс
    // R-8 ТЗ по #92: возможность модуля доказывает маркер другого модуля).
    since: "f31e9d6",
    ...
    async run(options, fixtures) {
      ...
      // Четвёртая половина (824a3e8, #164): служебный ключ privacy_column_decisions
      // снимается на выходе и клиенту не уходит НИКОГДА. Раньше его снимал только
      // обход маскирования, поэтому при выключенном рубильнике ключ уезжал наружу
      // (BUH, 14.08), а с ранним выходом R-2 уезжал бы и при включённом.
      // Проверка на успешном запросе со строками: ключ ставится при полном разборе
      // пометок, то есть именно там, где есть что разбирать.
      if (fixtures.catalog) {
        const строки = await callTool(options, "run_1c_query", {
          query: `ВЫБРАТЬ ПЕРВЫЕ 3 Т.Ссылка КАК Ссылка ИЗ ${fixtures.catalog} КАК Т`,
          limit: 3,
        });
        if (строки?.ok === true
          && Object.prototype.hasOwnProperty.call(строки, "privacy_column_decisions")) {
          return { status: "fail", note: "в ответе остался служебный ключ privacy_column_decisions —"
            + " MCP_Tools до снятия ключа на выходе (#164)" };
        }
      }
      return { status: "pass", note: `stage=${stage}, correlation_id есть, предупреждений политики ${состав} счётчиком,`
        + " служебного ключа в ответе нет" };
    },
  },
  {
    module: "MCP_Query",
    ...
    // since=7388bf4: R-7 — пометки колонок считаются один раз на результат, а не
    // на каждую ячейку. Признака снаружи нет (вердикты те же), свежесть модуля
    // доказывают ревизия и падение времени на маркере MCP_Security.
    ...
    // списка для показа.
    // since=d1364e3: антиомоглиф снят с сервера (#145, ревизия 2026-08-17.1) —
    // признак добавлен вторым условием ниже: запрос к Справочник.MCP_Маскирование
    // не получает temporary_table_identifier_mixed_script.
    // since=4832b71: запись domain_guidance_unavailable переведена на метрики
    // (стадия и длина вместо текста) — хвост волны a93f799, тот же класс: журнал
    // наружу не отдаётся, признака нет. Свежесть доказывает ревизия 2026-08-17.2,
    // содержимое записи — просмотр ЖР изнутри 1С.
    ...
    // since=2654b0a: защита ВЫБОР…ЕСТЬNULL признаётся (#150 Д-5, 2026-08-17.7) —
    // признак добавлен четвёртым условием ниже, с контролем на незащищённом поле.
    since: "2654b0a",
    ...
    async run(options, fixtures) {
      ...
      // Второе условие: правило антиомоглифа снято с сервера (#145, ревизия
      // 2026-08-17.1). Признак поведенческий и парный к ревизии: на сборке до
      // снятия temporary_table_identifier_mixed_script приходит на ЛЮБОЙ запрос
      // к этому имени. Проверяется именно код, а не valid: allowlist и права
      // меняет администратор, а не публикация, и другие коды маркеру не сигнал.
      const политика = await callTool(options, "validate_1c_query", {
        query: "ВЫБРАТЬ ПЕРВЫЕ 1 Т.Ссылка КАК Ссылка ИЗ Справочник.MCP_Маскирование КАК Т",
        strict: true,
      });
      const кодыПолитики = (политика?.errors ?? []).map((item) => item.code);
      if (кодыПолитики.includes("temporary_table_identifier_mixed_script")) {
        return { status: "fail", note: "антиомоглиф всё ещё блокирует Справочник.MCP_Маскирование — "
      ...
      // Четвёртое условие (#150 Д-5, ревизия 2026-08-17.7): поле в блоке ВЫБОР,
      // где то же поле уже под ЕСТЬNULL, защищено конструкцией — обёртка с
      // недостижимой заглушкой не требуется. Пакет самодостаточен (Валюты есть
      // на всех контурах). Контроль на незащищённом поле держит правило живым.
      const защищённый = await callTool(options, "validate_1c_query", {
      ...
      const кодыЗащищённого = (защищённый?.errors ?? []).map((item) => item.code);
      if (кодыЗащищённого.includes("outer_join_field_without_isnull")) {
        return { status: "fail", note: "поле в ИНАЧЕ при защите ЕСТЬNULL в КОГДА всё ещё требует обёртку — "
          + "опубликован MCP_Query ДО #150 Д-5 (ревизия 2026-08-17.7)" };
      }
      const незащищённый = await callTool(options, "validate_1c_query", {
        query: "ВЫБРАТЬ Т.Ссылка КАК Ссылка, Т.Наценка КАК Наценка ПОМЕСТИТЬ ВТОснова ИЗ Справочник.Валюты КАК Т; "
          + "ВЫБРАТЬ Т.Ссылка КАК Ссылка, Т.Наценка КАК Наценка ПОМЕСТИТЬ ВТДоп ИЗ Справочник.Валюты КАК Т; "
      ...
      const кодыНезащищённого = (незащищённый?.errors ?? []).map((item) => item.code);
      if (!кодыНезащищённого.includes("outer_join_field_without_isnull")) {
        return { status: "fail", note: "правило outer_join_field_without_isnull молчит на незащищённом поле — "
          + "контроль #150 Д-5 не прошёл, правило снято вместо починки" };
      }

      // Пятое условие: текст правила обязан говорить на языке ТОГО ЖЕ вида
  ...
  },
  {
    ...
    // since=f31e9d6: GetAuditLog разбирает event_names (#158). Карта счетов этого
    // не доказывает — признак by_event проверяет маркер MCP_Audit «тайминги
    // наружу» (кейс R-8 по #92); здесь движется только отметка свежести.
    // since=2ed3b6d: #119 Д-2 — признак у собственного маркера поиска ниже;
    // здесь движется только отметка свежести.
    // since=c47ddb6: диагноз устаревшего снимка tools/list (2026-08-17.9) —
    // признак у маркера паспорта ниже; здесь движется только отметка свежести.
    since: "c47ddb6",
  ...
  },
  {
    ...
    // since=f31e9d6: модуль правлен #158 (разбор event_names в GetAuditLog) —
    // отметка движется, признак проверяет маркер MCP_Audit «тайминги наружу».
    // since=2ed3b6d: #119 Д-2 — признак у собственного маркера поиска ниже.
    // since=c47ddb6: диагноз устаревшего снимка tools/list (2026-08-17.9) —
    // признак добавлен третьим условием ниже, с контролем на имени вне
    // удалённой схемы.
    since: "c47ddb6",
    what: "паспорт пересобран: перепись метаданных отвечает, сокращённый без данных и с диагнозом снимка",
    async run(options) {
      ...
      // Третье условие (2026-08-17.9): имя из удалённой пересборкой схемы несёт
      // диагноз устаревшего снимка tools/list, имя вне схемы — не несёт.
      const диагноз = (r) => (r?.warnings ?? [])
        .some((w) => String(w).includes("tools/list") && String(w).includes("устаревш"));
      const староеИмя = await callTool(options, "get_database_passport", { include_information_registers: true });
      if (!диагноз(староеИмя)) {
        return { status: "fail", note: "имя удалённой схемы не подняло диагноз снимка tools/list"
          + " — опубликован MCP_Tools_Impl до 2026-08-17.9" };
      }
      const внеСхемы = await callTool(options, "get_database_passport", { bogus_probe_flag: true });
      if (диагноз(внеСхемы)) {
        return { status: "fail", note: "диагноз снимка поднялся на имени вне удалённой схемы — отпечаток дырявый" };
      }
      ...
      return { status: "pass", note: `перепись: 11 видов, регистров сведений ${регистры};`
        ...
        + " диагноз снимка tools/list на месте, контроль чист" };
    },
  },
  {
    module: "MCP_Config",
    ...
    // since=14eb179: ревизия поднята до 2026-08-14.6 (pre-flight не строит
    // список полей для показа на успешном пути). Довод тот же: ревизия живёт в
    // этом модуле, сверка `код == контур` доказывает его свежесть точно.
    // since=0b17151: коммит ревизии .14.7 идёт ПОСЛЕ правки модулей — отметка
    // обязана указывать на него, иначе аудит свежести справедливо ругается.
    // since=d1364e3: ревизия поднята до 2026-08-17.1 (антиомоглиф снят с сервера,
    // #145). Довод прежний: ревизия живёт в этом модуле, сверка `код == контур`
    // доказывает его свежесть точно.
    // since=4832b71: ревизия поднята до 2026-08-17.2 (журнал без текстов
    // исключений — хвост волны .14.3). Довод тот же.
    ...
    // since=2654b0a: ревизия поднята до 2026-08-17.7 (#150 Д-5). Довод тот же.
    // since=c47ddb6: ревизия поднята до 2026-08-17.9 (диагноз устаревшего снимка
    // tools/list; .8 занят параллельной волной #177). Довод тот же.
    since: "c47ddb6",
  ...
  },
  {
    ...
    // since=7388bf4: и ревизией .14.5 (снятие производных закрытий + R-7/R-8).
    // since=14eb179: и ревизией .14.6 (pre-flight без списка для показа).
    // since=d1364e3: и ревизией 2026-08-17.1 (антиомоглиф снят с сервера, #145).
    // since=4832b71: и ревизией 2026-08-17.2 (журнал без текстов исключений).
    ...
    // since=2654b0a: и ревизией 2026-08-17.7 (#150 Д-5).
    // since=c47ddb6: и ревизией 2026-08-17.9 (диагноз устаревшего снимка tools/list).
  ...
  },
  {
    ...
    // since=90f5ea0: ранний выход маскирования (R-2). У этой правки признак
    // наблюдаемый и свой — стадия masking в событии аудита падает до единиц
    // миллисекунд на ответе, в котором политике нечего закрывать. Прежняя сборка
    // обходила такой ответ целиком: замеры 14.08 дают 43 мс на паспорте и
    // 1420-1572 мс на get_audit_log при живом рубильнике.
    // since=850e5fa: предпроверка перестала считать самоописание политики поводом
    // к обходу. Поймано этим же маркером на живом контуре — до приёмки.
    // since=7388bf4: R-8 — контейнер без изменений не пересобирается. Порог ниже
    // держится с запасом: правка только удешевляет тот же обход.
    // since=139ef0d: решение о допуске поля разделено на контекст объекта и поле
    // в контексте (.14.8). Тот же довод: вердикты прежние, цена ниже.
    since: "139ef0d",
    what: "ранний выход маскирования: masking≈0 на ответе без закрытых типов; вызовы не блокируются",
    async run(options) {
      ...
      // Рубильник выключен — второй эшелон и раньше выходил первой строкой, признак
      // ревизии неотличим от состояния контура. Это состояние (BUH 14.08), а не сбой.
      if (r?.privacy?.enabled !== true) {
        return { status: "pass", note: "вызовы проходят; ранний выход не проверить — рубильник privacy выключен" };
      }

      // Паспорт: к данным не обращается, закрытых типов в ответе нет по построению.
      const passport = await callTool(options, "get_database_passport", {});
      if (passport?.ok !== true || typeof passport.correlation_id !== "string") {
        return { status: "fail", note: "get_database_passport не ответил ok=true с correlation_id" };
      }
      await new Promise((s) => setTimeout(s, 1800));
      const audit = await callTool(options, "get_audit_log", {
        minutes_back: 10, correlation_id: passport.correlation_id, include_http: false, limit: 5,
      });
      ...
      const masking = event?.stages?.masking;
      if (typeof masking !== "number") {
        return { status: "fail", note: "в событии паспорта нет stages.masking — MCP_Audit старше стадий" };
      }
      // Порог откалиброван по живому замеру 14.08, а не по ожиданию: прежняя сборка
      // на этом ответе давала 43-47 мс, новая — 16-17. Ноль недостижим и не нужен:
      // предпроверка сама обходит ответ, и её цена остаётся в этой же стадии.
      // 30 — середина между двумя состояниями, различает надёжно.
      if (masking > 30) {
        return { status: "fail", note: `masking=${masking} мс на ответе без закрытых типов —`
          + " MCP_Security до раннего выхода R-2 (2026-08-14.4)" };
      }
      return { status: "pass", note: `вызовы проходят; ранний выход работает: masking=${masking} мс на паспорте` };
    },
  },
  {
    module: "MCP_Tools_Impl",
    // Маркер введён волной #119 Д-2 (ревизия 2026-08-17.5). Признак
    // поведенческий: явное поле «Представление» есть в опубликованном составе
    // любого справочника, но ПОДОБНО к нему платформа отвергает. Сборка ДО
    // волны отвечает предупреждением с текстом платформы «Неверные параметры»,
    // сборка ПОСЛЕ — отсевом с причиной «не строковые» без обращения к платформе.
    // since=c47ddb6: диагноз устаревшего снимка tools/list (2026-08-17.9) —
    // признак у маркера паспорта выше; здесь движется только отметка свежести.
    since: "c47ddb6",
    what: "search_objects: ПОДОБНО только по строковым полям (#119 Д-2)",
    async run(options, fixtures) {
      if (!fixtures.catalog) return { status: "skip", note: "нет справочника для пробы" };
      const r = await callTool(options, "search_objects", {
        query: "проба_гейта", types: [fixtures.catalog],
        search_fields: ["Представление"], limit: 1,
      });
      if (r?.ok !== true) {
        return { status: "fail", note: `search_objects не ответил: ${r?.error_code ?? "нет ok"}` };
      }
      const предупреждения = (r.warnings ?? []).join(" | ");
      if (/Неверные параметры/i.test(предупреждения)) {
        return { status: "fail", note: "ПОДОБНО по «Представление» дошёл до платформы — "
          + "опубликован MCP_Tools_Impl ДО #119 Д-2 (ревизия 2026-08-17.5)" };
      }
      if (!/не строковые|ПОДОБНО/i.test(предупреждения)) {
        return { status: "fail", note: "отсев нестрокового поля не объяснён предупреждением — "
          + `warnings: ${предупреждения.slice(0, 120) || "пусто"}` };
      }
      return { status: "pass", note: "нестроковое поле отсеяно с причиной, платформа не тронута" };
    },
  },
  {
    module: "MCP_Examples",
    // since=4832b71: событие MCP.QueryExample.skip несёт reason/error_chars вместо
    // сырого текста под debug_skip (запрет данных в журнале, 2026-08-17.2).
    // Снаружи признака нет: механизм выключен настройкой на всех контурах, а
    // skip-события инструментами не читаются (#158). Свежесть доказывает ревизия,
    // содержимое — просмотр ЖР изнутри 1С. Возможность маркера прежняя.
    since: "4832b71",
  ...
  },
  {
    ...
    // since=f31e9d6: точечный запрос имён событий (#158) — признак by_event у
    // маркера «тайминги наружу»; здесь снова движется только отметка.
    // since=c9bcdf2: дофикс by_event (.17.4) — признак там же, отметка следом.
    since: "c9bcdf2",
  ...
  },
  {
    ...
    // since=f31e9d6: event_names/by_event (#158, ревизия 2026-08-17.3) — признак
    // добавлен последним условием этого маркера и доказывает пару
    // MCP_Tools_Impl (разбор аргумента) + MCP_Audit (агрегат by_event).
    // since=c9bcdf2: дофикс .17.4 — by_event отдаёт все запрошенные имена с
    // нулями; условие маркера ужесточено до точного числа элементов.
    since: "c9bcdf2",
  ...
  },
  {
    ...
    // since=f31e9d6: точечный запрос имён событий (#158) — признак by_event у
    // маркера «тайминги наружу»; здесь движется только отметка свежести.
    // since=c9bcdf2: дофикс by_event (.17.4) — признак там же, отметка следом.
    since: "c9bcdf2",
...
async function main() {
  ...
  if (unwrapped?.ok !== true && notFound) {
    ...
    // причиной, что и в фикстурах маркеров. До ревизии 2026-08-17.1 латиница
    // «MCP_» внутри кириллического имени срабатывала антиомоглифом до проверки
    // полей (правило снято с сервера — #145); латиница сортируется раньше
    // кириллицы, поэтому такой объект приходил из discovery ПЕРВЫМ и забирал
    // запасной запрос — на ЗУП (нет плана счетов) гейт из-за этого падал с
    // temporary_table_identifier_mixed_script и сообщал о частичной публикации,
    // которой не было. Исключение остаётся и после снятия правила: служебный
    // объект — не представительная фикстура для проверки живости.
