...
const MODULE_MARKERS = [
  ...
  },
  {
    ...
    async run(options, fixtures) {
      ...
      // stage лежит на верхнем уровне ответа и в error, а НЕ в error.details: MCP_Tools
      // копирует его именно туда, чтобы поле было видно клиенту. Проверка одного лишь
      // error.details.stage не проходила никогда и делала гейт неработоспособным.
      const stage = r?.stage ?? r?.error?.stage ?? r?.error?.details?.stage;
      return stage
        ? { status: "pass", note: `stage=${stage}` }
        : { status: "fail", note: "stage отсутствует и на верхнем уровне, и в error" };
    },
  },
  {
    module: "MCP_Query",
