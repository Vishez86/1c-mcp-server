...
const MODULE_MARKERS = [
  ...
  },
  {
    ...
    // since=865e0b2: ревизия поднята до 2026-08-17.8 (#177 — маски полей типов
    // действуют на поля табличных частей в гейте запроса). Довод тот же.
    since: "865e0b2",
  ...
  },
  {
    ...
    // since=865e0b2: и ревизией 2026-08-17.8 (#177 — поля ТЧ в гейте запроса).
  ...
  },
  {
    ...
    // since=865e0b2: #177 — ПолеЗакрытоPrivacy для полного имени ТЧ отвечает по
    // записи типа-владельца. Признак наблюдаем только при политике с помеченным
    // полем ТЧ И включённом рубильнике — это состояние контура, а не свойство
    // сборки (класс 91b31af), поэтому маркером не проверяется. Свежесть выводится
    // из ревизии 2026-08-17.8 (сверяется точно) и атомарной публикации комплекта;
    // живой признак — privacy_acceptance --enforced, секция «поля табличных
    // частей (#177)»: на ERP до публикации падали ровно её пробы (4 FAIL), после
    // публикации 25/0.
    since: "865e0b2",
