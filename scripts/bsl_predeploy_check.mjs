...
// ── E3. Литерал склеен с идентификатором: внутренняя кавычка не удвоена ──────
// Симптом тот же, что у E2 («Ожидается символ ')'»), причина другая: кавычка не
// экранирована НИКАК. В строке
//   "текст register_types=["РегистрРасчета"] дальше"
// кавычек чётное число и ни одного слэша — E и E2 молчат, а Конфигуратор видит
// три лексемы подряд: литерал, идентификатор, литерал. Ровно этот дефект прошёл
// предполёт 14.08 и был отклонён компилятором на MCP_Knowledge семью ошибками.
//
// Признак надёжен по построению: в cleanLines содержимое литералов заменено
// пробелами, а кавычки сохранены, и в корректном коде кавычка соседствует только
// с пробелом, оператором, скобкой, запятой или концом строки — никогда с буквой,
// цифрой или подчёркиванием. Замер 14.08: на всех 20 файлах комплекта ноль
// срабатываний, то есть правило не создаёт ложного шума.
const gluedLiteral = [];
cleanLines.forEach((l, i) => {
  if (/[\p{L}\p{N}_]"|"[\p{L}\p{N}_]/u.test(l)) gluedLiteral.push(i + 1);
});
report(gluedLiteral.length === 0,
  "внутренние кавычки удвоены: литерал не склеен с идентификатором",
  gluedLiteral.length
    ? `строки ${gluedLiteral.slice(0, 8).join(", ")} — внутри литерала пишите ""`
    : "");
