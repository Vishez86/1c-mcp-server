...
// Ревизия 2026-08-11.3: справочник людей с политикой «ФИО и паспортные поля»,
// без пометки Наименования. Второй — тот же состав плюс псевдоним типа: там
// производное закрытие представления обязано уступить коду.
const FIO_CAT = "Справочник.СотрудникиБезИмени";
const FIO_CAT_ALIAS = "Справочник.СотрудникиСПсевдонимом";
...
// #177: тип, у которого администратор пометил поля строк ТЧ. Политика хранит
// плоские имена — пометка закрывает одноимённые поля и шапки, и табличной части.
const TP_FIELDS_CAT = "Справочник.СПометкамиПолейТЧ";
const TP_FIELDS_TP = `${TP_FIELDS_CAT}.ТабличнаяЧасть.КонтактнаяИнформация`;
...
const POLICY = {
  ...
  [FIO_CAT]: { alias: false, fields: ["ФИО", "Фамилия", "Имя", "Отчество", "ИНН"] },
  [FIO_CAT_ALIAS]: { alias: true, fields: ["ФИО", "Фамилия", "Имя", "Отчество", "ИНН"] },
  [TP_FIELDS_CAT]: { alias: false, fields: ["Наименование", "Значение", "НомерТелефона"] },
};
...
const META = {
  ...
  [FIO_CAT]: { Наименование: null, Код: null, ФИО: null, Фамилия: null, Имя: null,
    Отчество: null, ИНН: null, Ссылка: FIO_CAT },
  [FIO_CAT_ALIAS]: { Наименование: null, Код: null, ФИО: null, Фамилия: null, Имя: null,
    Отчество: null, ИНН: null, Ссылка: FIO_CAT_ALIAS },
  ...
  [TP_FIELDS_CAT]: { Наименование: null, Код: null, Значение: null, Ссылка: TP_FIELDS_CAT },
  [TP_FIELDS_TP]: { Представление: null, Значение: null, ЗначенияПолей: null, АдресЭП: null,
    НомерТелефона: null, Тип: null, Вид: null, НомерСтроки: null },
};
...
// MCP_Security.ПолеЗакрытоPrivacy. РЕШЕНИЕ ЗАКАЗЧИКА 11.08.2026 (ревизия
// 2026-08-11.2): закрыто ровно то, что администратор пометил в обработке.
// Из модели убраны правила, расширявшие состав за него: R-11 (поля строки ТЧ по
// закрытому владельцу), R-12/R-15 (подчинённость персоне и набор ФИО), R-13
// (ядро идентифицирующих полей) и R-9 (наследование представления от любого
// закрытого реквизита). Кейсы, проверявшие их, переведены в контрольные с
// ожиданием «открыто» — теперь они стерегут само требование.
const fieldClosed = (type, field) => {
  const p = POLICY[type];
  if (!p) {
    // #177: запись политики для поля ТЧ лежит на ТИПЕ-ВЛАДЕЛЬЦЕ — обработка
    // хранит плоские имена, ключей вида «…ТабличнаяЧасть.…» в политике не
    // бывает. Для полного имени ТЧ предикат отвечает по записи владельца, как
    // всегда делал второй эшелон.
    const owner = tabularOwner(type);
    return owner ? fieldClosed(owner, field) : false;
  }
  if (p.fields.some((f) => normField(f) === normField(field))) return true;
  // Единственное сохранённое следствие пометки: `presentation` — служебное
  // представление объекта в ответе, несущее то же название, что помеченное
  // имя-подобное поле. Имя-подобный НАБОР при этом не раскрывается.
  if (["ПРЕДСТАВЛЕНИЕ", "PRESENTATION"].includes(normField(field)) && maskPresentationOf(type)) {
    return true;
  }
  // Ревизия 2026-08-11.3: пометка ИМЕНИ ПЕРСОНЫ (ФИО) закрывает весь класс имени
  // объекта — Наименование, НаименованиеПолное, КраткоеНаименование, ФИО и
  // представление: у справочника людей это одна и та же строка. Псевдониму не
  // уступает (иначе `ВЫБРАТЬ ФЛ.Наименование` без ссылки в строке ушёл бы
  // открытым); различимость при псевдониме даёт Код — он в класс имени не входит.
  return maskPersonNameOf(type) && OBJECT_NAME_CLASS.has(normField(field));
};
// MCP_Security.ПредставлениеЗакрытоPrivacy: псевдоним типа либо помеченное
// имя-подобное поле. Наследование R-9 и закрытие представления строки ТЧ по
// владельцу (R-11) сняты.
const presentationClosed = (type) => {
  const p = POLICY[type];
  if (!p) return false;
  // Ревизия 2026-08-11.3: mask_person_name закрывает представление наравне с
  // mask_presentation — у справочника людей ФИО и представление одна строка.
  return Boolean(p.alias) || maskPresentationOf(type) || maskPersonNameOf(type);
};
// MCP_Config: mask_presentation — признак «в fields есть имя-подобное поле».
const maskPresentationOf = (type) => {
  const p = POLICY[type];
  return Boolean(p) && p.fields.some((f) => NAME_LIKE_SET.has(normField(f)));
};
// MCP_Config: mask_person_name — «в fields есть имя персоны» (роль имя_персоны).
// Ревизия 2026-08-11.3. Обратной силы нет: пометка Наименования не закрывает ФИО.
const maskPersonNameOf = (type) => {
  const p = POLICY[type];
  return Boolean(p) && p.fields.some((f) => PERSON_NAME_SET.has(normField(f)));
};
const NAME_LIKE_SET = new Set(["НАИМЕНОВАНИЕ", "НАИМЕНОВАНИЕПОЛНОЕ", "ПОЛНОЕНАИМЕНОВАНИЕ",
  "КРАТКОЕНАИМЕНОВАНИЕ", "КОД", "ПРЕДСТАВЛЕНИЕ"].map(normField));
const PERSON_NAME_SET = new Set(["ФИО", "FIO"].map(normField));
// MCP_Security.ПолеКлассаИмениОбъектаPrivacy — Код сюда НЕ входит.
const OBJECT_NAME_CLASS = new Set(["НАИМЕНОВАНИЕ", "НАИМЕНОВАНИЕПОЛНОЕ", "ПОЛНОЕНАИМЕНОВАНИЕ",
  "КРАТКОЕНАИМЕНОВАНИЕ", "ПРЕДСТАВЛЕНИЕ", "PRESENTATION", "ФИО", "FIO"].map(normField));
...
const CASES = [
  ...
  ["R9-1 контроль (R-9 снят): Представление регистра без пометки имени — ОТКРЫТО",
    `ВЫБРАТЬ Т.Представление КАК П ИЗ ${MASKED_REG} КАК Т`, "П", "открыто"],
  ...
  ["R9-4 контроль (R-13 снят): имя типа с маской Комментарий — ОТКРЫТО",
  ...
  // ---- Ревизия 2026-08-11.3: ФИО закрывает представление ----
  // Замер трёх контуров 11.08: политика «ФИО, Фамилия, Имя, Отчество, ИНН» без
  // пометки Наименования отдавала ФИО открытым сразу тремя каналами. Причина —
  // роль «имя_персоны» не входила в триггер mask_presentation.
  ["F1 ПРЕДСТАВЛЕНИЕ ссылки при пометке ФИО — закрыто",
    `ВЫБРАТЬ ПРЕДСТАВЛЕНИЕ(Т.Ссылка) КАК П ИЗ ${FIO_CAT} КАК Т`, "П", "маска"],
  ["F2 Наименование того же типа — закрыто (имя-подобное следствие ФИО)",
    `ВЫБРАТЬ Т.Наименование КАК П ИЗ ${FIO_CAT} КАК Т`, "П", "маска"],
  ["F3 сама пометка ФИО работает",
    `ВЫБРАТЬ Т.ФИО КАК П ИЗ ${FIO_CAT} КАК Т`, "П", "маска"],
  ["F4 контроль: не помеченное поле того же типа остаётся открытым",
    `ВЫБРАТЬ Т.Код КАК П ИЗ ${FIO_CAT} КАК Т`, "П", "открыто"],
  // Псевдониму производное закрытие НЕ уступает: уступка сняла бы пометку колонки
  // гейтом запроса, и `ВЫБРАТЬ ФЛ.Наименование` без ссылки в строке ушёл бы
  // открытым. Различимость при псевдониме держится на Коде (см. F7).
  ["F5 тот же состав плюс псевдоним типа — представление всё равно закрыто",
    `ВЫБРАТЬ ПРЕДСТАВЛЕНИЕ(Т.Ссылка) КАК П ИЗ ${FIO_CAT_ALIAS} КАК Т`, "П", "код псевдонима"],
  ["F6 и Наименование при псевдониме — закрыто",
    `ВЫБРАТЬ Т.Наименование КАК П ИЗ ${FIO_CAT_ALIAS} КАК Т`, "П", "код псевдонима"],
  ["F7 Код в класс имени НЕ входит: при псевдониме различимость сохраняется",
    `ВЫБРАТЬ Т.Код КАК П ИЗ ${FIO_CAT_ALIAS} КАК Т`, "П", "открыто"],
  ["F8 НаименованиеПолное того же класса — закрыто пометкой ФИО",
    `ВЫБРАТЬ Т.Наименование КАК П ИЗ ${FIO_CAT} КАК Т`, "П", "маска"],
  ...
  ["G2 контроль (R-11 снят): поле ТЧ без своей записи политики — ОТКРЫТО",
    `ВЫБРАТЬ ПЕРВЫЕ 5 К.КонтактнаяИнформация.Значение КАК П ИЗ ${CLOSED} КАК К`, "П", "открыто"],
  ["G2-2 контроль: поле ТЧ от ТЧ-источника — ОТКРЫТО",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.ЗначенияПолей КАК П ИЗ ${TP_CLOSED.replace(".ТабличнаяЧасть.", ".")} КАК Т`,
    "П", "открыто"],
  ["G2-3 контроль: телефон в той же ТЧ — ОТКРЫТ (закрывается своей пометкой)",
  ...
  // ---- #177: помеченные поля строк ТЧ (плоские имена записи владельца) ----
  // До правки гейт искал запись политики по полному имени ТЧ и не находил её
  // никогда: помеченные телефон и Значение уходили открытыми всеми тремя
  // путями — при закрытом presentation ссылки-владельца в той же строке.
  // Контрольные кейсы G2-2/G2-3 выше остаются в силе: там поля НЕ помечены.
  ["TF1 прямой ТЧ-источник: помеченный НомерТелефона — маска",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.НомерТелефона КАК П ИЗ ${TP_FIELDS_TP.replace(".ТабличнаяЧасть.", ".")} КАК Т`,
    "П", "маска"],
  ["TF2 прямой ТЧ-источник: помеченное Значение — маска",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.Значение КАК П ИЗ ${TP_FIELDS_TP.replace(".ТабличнаяЧасть.", ".")} КАК Т`,
    "П", "маска"],
  ["TF3 разыменование через сегмент ТЧ от шапки — маска",
    `ВЫБРАТЬ ПЕРВЫЕ 5 К.КонтактнаяИнформация.НомерТелефона КАК П ИЗ ${TP_FIELDS_CAT} КАК К`,
    "П", "маска"],
  ["TF4 соединение со своей ТЧ: поле строки — маска",
    `ВЫБРАТЬ Т.НомерТелефона КАК П ИЗ ${TP_FIELDS_CAT} КАК К СОЕДИНЕНИЕ `
      + `${TP_FIELDS_TP.replace(".ТабличнаяЧасть.", ".")} КАК Т`,
    "П", "маска"],
  ["TF5 Представление строки ТЧ — закрыто следствием mask_presentation владельца",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.Представление КАК П ИЗ ${TP_FIELDS_TP.replace(".ТабличнаяЧасть.", ".")} КАК Т`,
    "П", "маска"],
  ["TF6 контроль: непомеченное поле той же ТЧ (ЗначенияПолей) — открыто",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.ЗначенияПолей КАК П ИЗ ${TP_FIELDS_TP.replace(".ТабличнаяЧасть.", ".")} КАК Т`,
    "П", "открыто"],
  ["TF7 контроль: АдресЭП не помечен — открыт",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.АдресЭП КАК П ИЗ ${TP_FIELDS_TP.replace(".ТабличнаяЧасть.", ".")} КАК Т`,
    "П", "открыто"],
  ["TF8 то же плоское имя в шапке — закрыто той же записью",
    `ВЫБРАТЬ ПЕРВЫЕ 5 К.Значение КАК П ИЗ ${TP_FIELDS_CAT} КАК К`,
    "П", "маска"],
  ["TF9 контроль: классификатор Тип строки ТЧ — открыт",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.Тип КАК П ИЗ ${TP_FIELDS_TP.replace(".ТабличнаяЧасть.", ".")} КАК Т`,
    "П", "открыто"],
  // ---- R-12/R-15: подчинённые справочники персон (§4.14, §4.17) ----
  ["G3 контроль (R-12/R-15 сняты): Фамилия подчинённого без записи — ОТКРЫТА",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.Фамилия КАК П ИЗ ${PERSON_SUB} КАК Т`, "П", "открыто"],
  ["G14' контроль: Наименование подчинённого без записи — ОТКРЫТО",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.Наименование КАК П ИЗ ${PERSON_SUB} КАК Т`, "П", "открыто"],
  ["G14'-2 контроль: НаименованиеСлужебное подчинённого — ОТКРЫТО",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.НаименованиеСлужебное КАК П ИЗ ${PERSON_SUB} КАК Т`, "П", "открыто"],
  ["G14'' контроль: Имя подчинённого без записи — ОТКРЫТО",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.Имя КАК П ИЗ ${PERSON_SUB} КАК Т`, "П", "открыто"],
  ...
  ["G10 контроль: Отчество у типа персон НЕ помечено в политике — ОТКРЫТО",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.Отчество КАК П ИЗ ${PERSON} КАК Т`, "П", "открыто"],
  ...
  ["G4-син контроль: РасширенноеПредставлениеИНН не помечен — ОТКРЫТ (ядро снято)",
    `ВЫБРАТЬ ПЕРВЫЕ 5 Т.РасширенноеПредставлениеИНН КАК П ИЗ ${CLOSED} КАК Т`, "П", "открыто"],
