...
// Политика: закрытый справочник с псевдонимом типа и закрытым полем, плюс
// открытый справочник. Имена вымышленные — живых имён контура здесь быть не должно.
const CLOSED = "Справочник.ЗакрытыйТип";
const OPEN = "Справочник.ОткрытыйТип";

const POLICY = {
  [CLOSED]: { presentation: true, fields: ["Наименование", "НаименованиеПолное", "Код", "Представление"] },
};
...
// MCP_Security.ПолеЗакрытоPrivacy
const fieldClosed = (type, field) => {
  const p = POLICY[type];
  if (!p) return false;
  const norm = (s) => String(s).toUpperCase().replace(/[\s_\-.]/g, "");
  return p.fields.some((f) => norm(f) === norm(field));
};
// MCP_Security.ПредставлениеЗакрытоPrivacy
const presentationClosed = (type) => Boolean(POLICY[type]?.presentation);
...
// Порт ПодменаОперандовВыраженияPrivacy (R-2 + R-3)
