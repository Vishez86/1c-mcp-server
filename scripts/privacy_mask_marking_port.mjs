...
// когда у типа закрыт хотя бы один реквизит. Отдельный признак, не имя-подобный
// набор: Наименование и Код типа с одной строковой маской остаются открытыми.
const fieldClosed = (type, field) => {
  const p = POLICY[type];
  if (!p) return false;
  if (p.fields.some((f) => normField(f) === normField(field))) return true;
  return ["ПРЕДСТАВЛЕНИЕ", "PRESENTATION"].includes(normField(field)) && p.fields.length > 0;
};
// MCP_Security.ПредставлениеЗакрытоPrivacy: псевдоним типа, имя-подобное поле в
// масках либо наследование R-9 от любого закрытого реквизита.
const presentationClosed = (type) => {
  const p = POLICY[type];
  if (!p) return false;
  return Boolean(p.alias) || p.fields.length > 0;
};
...
const CASES = [
  ...
  // Цена R-9, зафиксированная кейсами: у типа с единственной не-имя-подобной
  // маской представление закрывается (наследование), а имя и код — НЕТ
  // (признак отдельный, имя-подобный набор не тянется — ловушка mask_presentation).
  ...
  ["R9-6 ПРЕДСТАВЛЕНИЕ ссылки того же типа — маска (цена наследования)",
    `ВЫБРАТЬ ПРЕДСТАВЛЕНИЕ(Т.Ссылка) КАК П ИЗ ${MASKED_CAT} КАК Т`, "П", "маска"],
];
