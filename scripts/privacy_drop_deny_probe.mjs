...
async function sectionTools() {
  ...
  if (!byRef.ok) record(S, "И2 get_object_by_ref", "SKIP", `транспорт: ${byRef.transport ?? ""}`);
    ...
    //
    // `Код` и `code` из перечня ИСКЛЮЧЕНЫ (правка 14.08). Кейс писался до
    // решения 11.08 и ждал их подмены, из-за чего давал FAIL на корректной
    // сборке. Код не имя, а стабильный идентификатор: через него обезличенные
    // строки остаются различимыми и связываемыми между запросами, поэтому он
    // закрывается ТОЛЬКО собственной пометкой администратора. Ревизия
    // 2026-08-14.4 убрала последнее место, где он закрывался производно
    // (псевдоним типа). Проверка кода вынесена в парный кейс И2б ниже:
    // «закрыто имя» и «открыт код» — два независимых требования, и молчать о
    // втором нельзя, иначе регресс различимости пройдёт незамеченным.
    const поля = { ...(o.standard_fields ?? {}), ...(o.fields ?? {}) };
    const именные = Object.entries(поля).filter(([k]) =>
      /^(наименование|наименованиеполное|полноенаименование|краткоенаименование|представление|name|description)$/i
    ...
    // И2б. Различимость: код объекта закрытого типа обязан остаться открытым,
    // если администратор не пометил его сам. FAIL здесь — не «мало закрыли», а
    // регресс: обезличенные строки перестали быть различимыми.
    const коды = Object.entries(поля).filter(([k]) =>
      /^(код|code)$/i.test(String(k).replace(/[\s_-]/g, "")))
      .filter(([, v]) => v !== null && v !== undefined && v !== "");
    if (!коды.length) {
      record(S, "И2б код открыт: различимость сохранена", "SKIP", "заполненного кода в ответе нет");
    } else {
      const закрытых = коды.filter(([, v]) => looksMasked(v)).length;
      check(S, "И2б код открыт: различимость сохранена", закрытых === 0,
        `кодов ${коды.length}, закрытых ${закрытых}`
        + (закрытых ? " — код закрыт производным правилом, а не пометкой" : ""));
    }
  }
