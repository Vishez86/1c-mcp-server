...
Важно для промежуточного `py_server`: желательно проксировать `structuredContent` без потерь. Сервер 1С дополнительно дублирует критичную диагностику в `content[]` строкой `Диагностика JSON: ...`, чтобы модель всё равно увидела причину отказа через реализации, которые передают только текстовый content.
