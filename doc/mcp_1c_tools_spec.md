...
- **Диагностика (dev-only, по умолчанию выключена).** При `query_examples.debug_skip=true` в ответ добавляется поле `diagnostics` — массив последних (до 50) событий `MCP.QueryExample.skip` с полями `at` (ISO-время) и `error` (текст сбоя стадии `СхемаЗапроса`, до 2000 симв.), а сам текст ошибки пишется в событие `MCP.QueryExample.skip` (уровень «Примечание»). Служит для отладки покрытия обезличивания и объектной модели `СхемаЗапроса`. **Важно:** текст ошибки может содержать неканонизированные идентификаторы исходного запроса, поэтому в продакшене `debug_skip` обязан быть `false` (тогда поле `diagnostics` отсутствует, а в skip-событие идентификаторы не пишутся).
