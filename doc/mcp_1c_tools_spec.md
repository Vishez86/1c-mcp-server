# Спецификация MCP-сервера для 1С: 23 read-only tools
...
`Справочник.Сотрудники*` стабильными псевдонимами вида `ФЛ-453276` или
`Сотр-453276`. Паспортные данные, индивидуальный номер, страховой номер и дата
рождения маскируются как чувствительные поля. Ссылки продолжают содержать
...
| 7 | `get_accounting_entries` | Получить бухгалтерские проводки с универсальным join к субконто | P0 |
| 8 | `get_inventory_balances_by_item` | Получить быстрые остатки товара по складам и организациям | P0 |
| 9 | `get_calculation_types_map` | Получить карту видов расчёта плана видов расчёта | P0 |
| 10 | `get_database_passport` | Получить паспорт фактических данных базы | P0 |
| 11 | `get_object_by_ref` | Получить объект по типу и UUID ссылки | P0 |
| 12 | `find_object_by_id` | Найти объект по UUID без знания типа | P0 |
| 13 | `search_objects` | Поиск объектов по строке, коду, номеру, ИНН, артикулу | P0 |
| 14 | `get_link_of_object` | Получить навигационную ссылку на объект | P1 |
| 15 | `find_references_to_object` | Найти ссылки на объект | P1 |
| 16 | `get_enum_values` | Получить значения перечисления | P0 |
| 17 | `get_register_records` | Получить записи, срезы, остатки и обороты регистров | P0 |
| 18 | `get_document_movements` | Получить движения документа по регистрам | P0 |
| 19 | `list_reports` | Получить список доступных отчётов | P1 |
| 20 | `get_report_info` | Получить параметры и структуру отчёта | P1 |
| 21 | `run_1c_report` | Выполнить отчёт 1С | P1 |
| 22 | `get_object_history` | Получить историю объекта, версии или события журнала | P2 |
| 23 | `get_current_user_context` | Получить текущий контекст пользователя и базы | P0 |
...
1. Все 23 tools возвращаются в `tools/list`.
...
Ожидание: `ok=true`, `read_only=true`, список tools содержит 23 tools.
