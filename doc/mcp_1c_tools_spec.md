# Спецификация MCP-сервера для 1С: 17 read-only tools
...
| 5 | `get_object_by_ref` | Получить объект по типу и UUID ссылки | P0 |
| 6 | `find_object_by_id` | Найти объект по UUID без знания типа | P0 |
| 7 | `search_objects` | Поиск объектов по строке, коду, номеру, ИНН, артикулу | P0 |
| 8 | `get_link_of_object` | Получить навигационную ссылку на объект | P1 |
| 9 | `find_references_to_object` | Найти ссылки на объект | P1 |
| 10 | `get_enum_values` | Получить значения перечисления | P0 |
| 11 | `get_register_records` | Получить записи, срезы, остатки и обороты регистров | P0 |
| 12 | `get_document_movements` | Получить движения документа по регистрам | P0 |
| 13 | `list_reports` | Получить список доступных отчётов | P1 |
| 14 | `get_report_info` | Получить параметры и структуру отчёта | P1 |
| 15 | `run_1c_report` | Выполнить отчёт 1С | P1 |
| 16 | `get_object_history` | Получить историю объекта, версии или события журнала | P2 |
| 17 | `get_current_user_context` | Получить текущий контекст пользователя и базы | P0 |
...
обобщённый ссылочный тип. При соединении независимых подзапросов по таким
полям рекомендуется выводить `УникальныйИдентификатор(Субконто...) КАК ...UUID`
и соединять подзапросы по UUID, оставляя саму ссылку отдельным полем для
представления.
...
ИЗ
    РегистрБухгалтерии.Хозрасчетный КАК х
СГРУППИРОВАТЬ ПО
    х.Регистратор
ИМЕЮЩИЕ
...
- РегистрБухгалтерии.<Имя>.Остатки(...) для остатков по счетам и субконто;
  ...
  аналитики остатков в разрезе доступных субконто. При JOIN между независимыми
...
1. Все 17 tools возвращаются в `tools/list`.
...
Ожидание: `ok=true`, `read_only=true`, список tools содержит 17 tools.
