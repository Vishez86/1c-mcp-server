...
- **`event_names` — точечный запрос произвольных имён `MCP.*`** (до 20 за вызов, только префикс `MCP.` — чужие события журнала через инструмент недостижимы). Список **заменяет** отбор по `tools`/`outcome`/`include_http`; ответ дополняется агрегатом `by_event` (`event`, `count`, `last_at`), который присутствует и при деградациях — его наличие отличает сборку с поддержкой аргумента от старой, молча его игнорирующей. События вне схемы tool/http приходят с `kind: "event"` и только счётными полями белого списка (`reason`, `error_chars`, `query_chars`, `param_count`); комментарий события не возвращается ни сырым, ни разобранным, событие с текстовым (не-JSON) комментарием счётно без разбора.
