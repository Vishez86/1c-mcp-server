...
нет, тип берётся из любой колонки, значение которой является ссылкой: в строке
`run_1c_query` колонку со ссылкой называет автор запроса (`КАК Ссылка`), и без
этого шага маски по типу к плоским колонкам строки не применялись бы. При
нескольких ссылках в узле приоритет у типа, настроенного в политике; при
нескольких настроенных — порядок обхода ключей. Поля и строки внутри объекта
