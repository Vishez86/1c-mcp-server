...
```json
{
  ...
  "metadata_counts": {
    "Константы": {"total": 745, "allowed": 745},
    "Справочники": {"total": 725, "allowed": 725},
    "Документы": {"total": 445, "allowed": 445},
    "Перечисления": {"total": 1162, "allowed": 1162},
    "Обработки": {"total": 394, "allowed": 394},
    "ПланыСчетов": {"total": 1, "allowed": 1},
    "ПланыВидовРасчета": {"total": 3, "allowed": 3},
    "РегистрыСведений": {"total": 1316, "allowed": 1316},
    "РегистрыНакопления": {"total": 107, "allowed": 107},
    "РегистрыБухгалтерии": {"total": 1, "allowed": 1},
    "РегистрыРасчета": {"total": 0, "allowed": 0}
  },
...
Числа в примере — замер 14.08.2026 на BUH_KORP (итого 4 899 объектов; ZUP_CORP 3 844, ERP_DEMO 7 294). Замер снят через `list_metadata_objects`, то есть это значения **`allowed`**; `total` в примере показан равным им лишь потому, что независимого замера сырых счётчиков конфигурации ещё нет — там, где allowlist или права скрывают объекты, `total` будет больше, и разность как раз и есть ответ на «сколько скрыто политикой». Ожидать равенства двух чисел на приёмке нельзя; проверяется `allowed ≤ total` и совпадение `allowed` с `total_estimated`.
...
- **Два числа на вид.** `total` — всего в конфигурации, `allowed` — доступно по allowlist и правам 1С. `allowed` сходится с `total_estimated` у `list_metadata_objects` по тому же виду: одно множество, посчитанное двумя путями. Разница `total − allowed` отвечает на «сколько скрыто политикой» без второго вызова.
