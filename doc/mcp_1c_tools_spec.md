...
- **Аргументы вызовов не возвращаются никогда и ни в каком режиме.** Аудит пишет в журнал `arguments.raw_json` — текст запроса и параметры, где отредактированы только ключи вида `password`/`token`/`secret`. Privacy-маскирование применяется к результату вызова, а не к его аргументам, поэтому выдача этого поля открыла бы канал в обход маскирования. По той же причине не возвращается `error.message`: сообщения платформы цитируют фрагмент упавшего запроса. Отдаётся только `error_code`.
