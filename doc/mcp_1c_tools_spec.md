...
- **Три разных `duration_ms` не взаимозаменяемы.** В ответе инструмента — выполнение запроса; в событии `MCP.<tool>.*` — весь вызов; в `MCP.http_request` — весь HTTP-запрос. По одному `correlation_id` они должны идти по возрастанию; HTTP-запись `correlation_id` не содержит и связывается по времени.
...
- **Различимость деградаций:** при отсутствии права `ПросмотрЖурналаРегистрации` — `source_available: false` и предупреждение `event_log_not_available`; при сбое чтения — `event_log_read_failed`. Пустой массив без флага читался бы как «вызовов не было».
