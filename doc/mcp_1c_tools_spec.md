...
- Запретить `ВЫБРАТЬ *` / alias wildcard (`Т.*`) и пакетные запросы.
...
- Запретить ПОМЕСТИТЬ, ИНДЕКСИРОВАТЬ ПО, УНИЧТОЖИТЬ, ДЛЯ ИЗМЕНЕНИЯ.
...
Функция ВыполнитьToolCall(ИмяТула, Аргументы, Контекст) Экспорт
    ...
    Попытка
        Результат = MCP_Tools.ВыполнитьИнструмент(ИмяТула, Аргументы, Контекст);
