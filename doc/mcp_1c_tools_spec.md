...
- Всегда запускать validate_1c_query, если validate_before_run=true.
...
- Потенциально запретить ПОМЕСТИТЬ, ИНДЕКСИРОВАТЬ ПО, УНИЧТОЖИТЬ, ДЛЯ ИЗМЕНЕНИЯ, пакетные запросы.
...
Функция ВыполнитьToolCall(ИмяТула, Аргументы, Контекст) Экспорт
    ...
    Попытка
        Результат = MCP_Tools.Выполнить(ИмяТула, Аргументы, Контекст);
