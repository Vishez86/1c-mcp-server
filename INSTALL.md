...
| `organization_aliases.enabled: true` | Вместо полных названий `Справочник.Организации` LLM получает стабильные псевдонимы вида `Орг-453276`. UUID, тип ссылки и `navigation_url` сохраняются, чтобы пользователь мог открыть объект в 1С и увидеть название по своим правам. |
...
| `person_aliases.enabled: true` | Вместо ФИО ссылок `Справочник.ФизическиеЛица` и `Справочник.Сотрудники*` LLM получает стабильные псевдонимы вида `ФЛ-453276` и `Сотр-453276`. UUID, тип ссылки и `navigation_url` сохраняются для открытия объекта в 1С. |
