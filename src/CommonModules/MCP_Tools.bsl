...
Функция ВыполнитьИнструмент(ИмяТула, Аргументы) Экспорт
	...
	Попытка
		...
		ДобавитьКонтекстАвторизации(Данные);
...
Процедура ДобавитьКонтекстАвторизации(Данные)
...
Функция Tool_run_1c_query()
	...
	Возврат _Tool("run_1c_query",
		...
		"Запрос на языке запросов 1С с параметрами. Постоянные данные строго read-only; временные таблицы в пакете разрешены через ПОМЕСТИТЬ/ИНДЕКСИРОВАТЬ ПО/УНИЧТОЖИТЬ. Для отчетной аналитики используйте быстрый путь: узкий search/list по сущностям, затем специализированные карты (get_accounting_accounts_map/get_calculation_types_map) и только выбранный источник. Сначала проверьте list_reports с query и малым limit; если найден готовый отчет, спросите пользователя: отчет или прямой запрос. Финальная команда пакета должна быть ВЫБРАТЬ. Для условий по агрегатам используйте ИМЕЮЩИЕ; при JOIN по Субконто из независимых подзапросов используйте УникальныйИдентификатор(...). Ссылки, даты и массивы в parameters передавайте как QueryParameterValue с kind=ref/datetime/array.",
...
Функция Tool_get_1c_query_guidance()
	...
	Возврат _Tool("get_1c_query_guidance",
		...
		"Возвращает встроенные универсальные правила из doc/skills: синтаксис, функции, временные таблицы, составные типы, субконто, быстрый путь отчетной аналитики, оптимизация и ограничения read-only MCP. Используйте перед сложными запросами и при исправлении ошибок validate_1c_query.",
		Props);
КонецФункции
...
Функция _Tool(Имя, Title, Описание, Properties, Required = Неопределено)
	...
	Описание = Описание + " Не выдумывайте методы, сущности, таблицы и поля: перед использованием имён получайте их через discovery/metadata tools или из результата этого tool. Права зависят от текущей учетной записи 1С; не кэшируйте сведения о доступе между перелогинами и при access_denied не повторяйте тот же запрос без смены пользователя или выдачи прав.";
	ОписаниеМаскирования = MCP_Config.ОписаниеМаскированияLLM();
	Если НЕ ПустаяСтрока(ОписаниеМаскирования) Тогда
		Описание = Описание + " " + ОписаниеМаскирования;
	КонецЕсли;
