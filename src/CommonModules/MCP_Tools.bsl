...
Функция Tool_get_database_passport(РежимРезультата = "")
	...
	Props.Вставить("include_accumulation_registers", _Схема("boolean", , "ТЯЖЁЛАЯ СЕКЦИЯ, запрашивается отдельным вызовом. Проверить регистры накопления на наличие данных. Вместе с include_information_registers НЕ передавать: два перебора не укладываются в таймаут публикации."));
	Props.Вставить("include_information_registers", _Схема("boolean", , "ТЯЖЁЛАЯ СЕКЦИЯ, запрашивается отдельным вызовом. Проверить регистры сведений на наличие данных и период, если есть поле Период. Вместе с include_accumulation_registers НЕ передавать."));
	...
	Возврат _Tool("get_database_passport",
		...
		+ " ЗАПРАШИВАЙТЕ ПАСПОРТ ТРЕМЯ ВЫЗОВАМИ, а не одним: (1) основная часть с нужными из include_organizations, include_period, include_closed_periods, include_calculation_registers;"
		+ " (2) отдельный вызов с include_information_registers; (3) отдельный вызов с include_accumulation_registers."
		+ " Перебор регистров сведений и накопления стоит десятки секунд каждый, вместе они превышают таймаут публикации, и ответ обрывается шлюзом БЕЗ структурированной ошибки."
		+ " Передача обоих флагов сразу отклоняется с кодом invalid_arguments.",
		Props, Неопределено, РежимРезультата);
КонецФункции
