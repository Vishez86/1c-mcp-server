...
Функция ВыполнитьИнструмент(ИмяТула, Аргументы, РежимРезультата = "", ВключитьКонтекстАвторизации = Неопределено) Экспорт
	...
	// Сбой самой проверки privacy трактуется как отказ. Оба блока ниже стоят до
	// общего Попытка, поэтому необработанное исключение уходило бы клиенту
	// протокольной internal_error без записи в аудит; privacy не может «не
	// сработать» молча, иначе ошибка внутри проверки открыла бы данные.
	ПричинаЗапрета = "";
	Попытка
		...
		КонецЕсли;
	Исключение
		ОшибкиPrivacy = Новый Массив;
		ОшибкиPrivacy.Добавить("проверка privacy-политики завершилась ошибкой: "
			+ ОписаниеОшибки());
	КонецПопытки;
	...
	// Причина уже вычислена выше, внутри защищённого блока.
