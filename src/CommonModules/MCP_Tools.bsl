...
Функция ВыполнитьИнструмент(ИмяТула, Аргументы, РежимРезультата = "", ВключитьКонтекстАвторизации = Неопределено) Экспорт
	...
	Попытка
		...
		Иначе
			...
			БезопасноЗаписатьАудитОшибки(CorrelationID, "unknown_tool", Новый Структура,
				ОшибкаАудита, СтартВыполненияМс, УровеньЖурналаРегистрации.Предупреждение);
			Возврат СформироватьОшибкуРезультата(
		...
		БезопасноЗаписатьАудитУспеха(CorrelationID, ИмяТула, Аргументы, ДанныеДляLLM, СтартВыполненияМс);
	...
	Исключение
		...
		БезопасноЗаписатьАудитОшибки(CorrelationID, ИмяТула, Аргументы, Ошибка, СтартВыполненияМс);
...
Функция Tool_get_audit_log(РежимРезультата = "")
	...
	Возврат _Tool("get_audit_log",
		...
		"Длительности вызовов инструментов и HTTP-запросов, записанные аудитом сервера: duration_ms по каждому вызову, correlation_id для связи с ответом, агрегат by_tool (calls, errors, min/p50/max). Дополняет duration_ms из ответа run_1c_query, который покрывает только выполнение запроса в СУБД: длительность вызова целиком и длительность HTTP-запроса видны только здесь, как и отказы access_denied/unknown_tool, не попадающие в ответ. Аргументы вызовов и тексты ошибок не возвращаются. Порядок и полнота окна платформой не гарантируются: при упоре в scan_limit возвращается scan_truncated.",
		Props, Неопределено, РежимРезультата);
КонецФункции
