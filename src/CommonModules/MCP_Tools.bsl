...
Функция Tool_run_1c_query()
	...
	Возврат _Tool("run_1c_query",
		...
		"Запрос на языке запросов 1С с параметрами. Постоянные данные строго read-only; временные таблицы в пакете разрешены через ПОМЕСТИТЬ/ИНДЕКСИРОВАТЬ ПО/УНИЧТОЖИТЬ. Для отчетной аналитики сначала проверьте list_reports и, если найден готовый отчет, спросите пользователя: отчет или прямой запрос. Финальная команда пакета должна быть ВЫБРАТЬ. Для условий по агрегатам используйте ИМЕЮЩИЕ; при JOIN по Субконто из независимых подзапросов используйте УникальныйИдентификатор(...).",
...
Функция Tool_get_1c_query_guidance()
	Props = Новый Структура;
	Props.Вставить("topic", _Схема("string", , "Тема или auto: metadata-first, query-structure, virtual-tables, temporary-tables, joins, grouping-and-having, functions, compound-types, subconto, null-handling, parameters, reports-vs-query, performance-pitfalls, mcp-query-safety."));
...
Функция Tool_list_reports()
	...
	Возврат _Tool("list_reports",
		...
		"Discovery tool для отчётов. Для отчетной аналитики может вернуть interaction_hint report_or_direct_query_choice: агенту следует спросить пользователя, использовать найденный отчет или строить прямой read-only запрос.",
		Props);
КонецФункции
