...
Функция ДиагностикаИзОшибокВалидации(ОшибкиВалидации)
	...
	Если ТипЗнч(ОшибкиВалидации) <> Тип("Массив") ИЛИ ОшибкиВалидации.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;

	ПерваяОшибка = ОшибкиВалидации[0];
	Код = "";
	Сообщение = "";
	Локация = "";
	Попытка
		ПерваяОшибка.Свойство("code", Код);
		ПерваяОшибка.Свойство("message", Сообщение);
		ПерваяОшибка.Свойство("location", Локация);
	Исключение
		Возврат Неопределено;
	КонецПопытки;
	...
	Если ПустаяСтрока(Код) И ПустаяСтрока(Сообщение) Тогда
		Возврат Неопределено;
	КонецЕсли;
