...
Функция ВыполнитьИнструмент(ИмяТула, Аргументы, РежимРезультата = "", ВключитьКонтекстАвторизации = Неопределено) Экспорт
	...
	РежимРезультата = НормализоватьРежимРезультата(РежимРезультата);
	СтартВыполненияМс = MCP_Audit.ТекущиеМиллисекундыUTC();
	...
	Попытка
		...
		Иначе
			...
			// Не используем имя из входящего запроса как имя события ЖР: оно может быть произвольным.
			БезопасноЗаписатьАудитОшибки(CorrelationID, "unknown_tool", Аргументы, ОшибкаАудита, СтартВыполненияМс);
			Возврат СформироватьОшибкуРезультата(
...
Процедура БезопасноЗаписатьАудитУспеха(CorrelationID, ИмяТула, Аргументы, Данные, СтартВыполненияМс)
	Попытка
		ДлительностьМс = MCP_Audit.ТекущиеМиллисекундыUTC() - СтартВыполненияМс;
...
Процедура БезопасноЗаписатьАудитОшибки(CorrelationID, ИмяТула, Аргументы, Ошибка, СтартВыполненияМс)
	Попытка
		ДлительностьМс = MCP_Audit.ТекущиеМиллисекундыUTC() - СтартВыполненияМс;
