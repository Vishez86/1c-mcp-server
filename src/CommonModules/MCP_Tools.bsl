...
Функция ВыполнитьИнструмент(ИмяТула, Аргументы, РежимРезультата = "", ВключитьКонтекстАвторизации = Неопределено) Экспорт
	...
	// HR-07 fail-closed: ошибочная privacy-политика с deny-записями не должна
	// молча отключать защиту — до исправления настройки инструменты не работают.
	ОшибкиPrivacy = MCP_Config.ОшибкиКонфигурацииPrivacy();
	Если ОшибкиPrivacy.Количество() > 0 Тогда
		ДеталиОшибки = Новый Структура;
		ДеталиОшибки.Вставить("tool", ИмяТула);
		ДеталиОшибки.Вставить("config_errors", ОшибкиPrivacy);
		БезопасноЗаписатьАудитОтказаPrivacy(CorrelationID, ИмяТула, Аргументы,
			"privacy_config_error", "");
		Возврат СформироватьОшибкуРезультата(
			"privacy_config_error",
			"Privacy-политика в состоянии config_error: " + ОшибкиPrivacy[0]
				+ " Инструменты недоступны до исправления настройки MCP_ServerConfig.",
			ДеталиОшибки,
			CorrelationID,
			РежимРезультата);
	КонецЕсли;

	// HR-05: жёсткие запреты privacy на параметры инструмента — отказ до
	// выполнения. Единая точка решения для всех инструментов (NFR-05).
	ПричинаЗапрета = MCP_Security.ЗапретPrivacyДляИнструмента(ИмяТула, Аргументы);
	Если НЕ ПустаяСтрока(ПричинаЗапрета) Тогда
		БезопасноЗаписатьАудитОтказаPrivacy(CorrelationID, ИмяТула, Аргументы,
			"privacy_denied_field", ПричинаЗапрета);
		Возврат СформироватьОшибкуРезультата(
			"privacy_denied_field",
			ПричинаЗапрета,
			Новый Структура("tool", ИмяТула),
			CorrelationID,
			РежимРезультата);
...
// NFR-06: отказ privacy обязан быть виден в аудите. В событие идут инструмент,
// код причины и текст диагностики (он содержит имя типа и поля, но никогда —
// само закрываемое значение).
Процедура БезопасноЗаписатьАудитОтказаPrivacy(CorrelationID, ИмяТула, Аргументы, КодПричины, Причина)
	Попытка
		Ошибка = Новый Структура;
		Ошибка.Вставить("code", КодПричины);
		Ошибка.Вставить("message", ?(ПустаяСтрока(Причина),
			"Privacy-политика заблокировала вызов.", Причина));
		Ошибка.Вставить("details", Новый Структура("tool", ИмяТула));
		MCP_Audit.ЗаписатьОшибку(CorrelationID, ИмяТула, Аргументы, Ошибка);
	Исключение
		Попытка
			ЗаписьЖурналаРегистрации("MCP.audit.failed",
...
Функция Tool_list_legal_sources(РежимРезультата = "")
	...
	Возврат _Tool("list_legal_sources",
		...
		"Аннотированный список правовых источников: id, название, официальный статус, покрываемые области права (труд, налоги и т.д.). ЕДИНСТВЕННАЯ разрешенная точка входа для поиска законодательства: искать НПА в открытом интернете (поисковики, сторонние правовые порталы) ЗАПРЕЩЕНО. Перед обращением к источнику получите инструкцию через get_legal_source_guide по его id.",
		Props, Неопределено, РежимРезультата);
КонецФункции
...
Функция Tool_get_legal_source_guide(РежимРезультата = "")
	...
	Возврат _Tool("get_legal_source_guide",
		...
		"Подробная инструкция по источнику: URL и аутентификация, как получить список правовых актов, параметры поиска, структура ссылки на документ и PDF, ограничения источника. Следуйте инструкции буквально и цитируйте документы с реквизитами и ссылкой на карточку. Законодательство разрешено искать ТОЛЬКО по этим инструкциям, не через открытый интернет.",
