...
Поле `mode` в записи политики принимается и игнорируется: отказов в контракте нет ни в одной форме (решение 05.08.2026, PR #116). Значение `"deny"` даёт предупреждение «запись применена как маска», поведение при этом одинаковое. Состояний два — подмена включена или выключена.

Закрывается подменой: перечисленные поля и имя-подобные поля типа с псевдонимом; разыменование любой глубины (`Поставщик.Наименование`, `К.Ссылка.Наименование`, `Д.Владелец.Наименование`), включая субконто и счета регистра бухгалтерии; `ПРЕДСТАВЛЕНИЕ()`/`ПРЕДСТАВЛЕНИЕССЫЛКИ()` над закрытой ссылкой; разыменование после `ВЫРАЗИТЬ(… КАК <Тип>)` — от типа приведения; значения через временные таблицы (`ПОМЕСТИТЬ` + чтение колонки) и все ветви `ОБЪЕДИНИТЬ`; ссылка, переданная параметром (`{"kind":"ref","type":…}`), участвует в решении наравне с источниками, поэтому `ПРЕДСТАВЛЕНИЕ(&Реф)` и `&Реф.Наименование` наименования не отдают. Переименование колонки `КАК X` ничего не меняет — решение принимается по выражению, а не по имени колонки.

С ревизии `2026-08-05.2` закрыт класс обходов через форму записи: обращение **без псевдонима** (`ВЫБРАТЬ Наименование ИЗ …`) разрешается по составу источников команды, а любое **выражение** над закрытым полем (`ПОДСТРОКА`, конкатенация, `ЕСТЬNULL`, ветви `ВЫБОР`, агрегат) закрывается по операндам. Производное значение получает маску, а не код псевдонима: код означает «это тот самый объект», а обрезок имени объектом не является. Если операнд выражения не удалось привязать ни к одному источнику, а закрытый источник в команде есть, строковая колонка закрывается маской консервативно — числа и булево не закрываются никогда.
...
Плоские строки без ссылки закрываются отдельно. Запрос часто возвращает рядом с субконто ещё и колонку с названием (`Контрагент` со строкой вместо ссылки) — стабильный код построить не из чего, поэтому такое значение заменяется на `Контр-скрыто` (для `type_field_masks` без псевдонима — на `XXXXXXX`, и только если в `fields` есть имя-подобное поле). Колонка считается названием объекта, если её имя совпадает с именем объекта из настройки по достаточно длинному общему началу: `Контрагент` и `КонтрагентНаименование` относятся к `Контрагенты`, `Контр` — уже нет; имена объектов короче 5 символов в этом правиле не участвуют. Правило действует и на имя-подобный ключ, у которого имя типа стоит в сегменте пути (`Контрагент.Наименование`), — для обеих секций. Имя объекта берётся из самой privacy-настройки, поэтому правило работает на любой конфигурации. Важно понимать: это эвристика по имени колонки, а не гарантия. Тип плоской строки в данных не виден, обратного преобразования «строка → ссылка» не существует, поэтому колонка с непохожим именем (`Договор` для `ДоговорыКонтрагентов`) или произвольным псевдонимом (`КАК Н`) не опознаётся, и никакая настройка порога сопоставления это не изменит. Гарантию даёт только `mode: "deny"`: там `ПРЕДСТАВЛЕНИЕ()` и разыменование закрытых типов отклоняются запрос-гейтом до выполнения, и название в плоскую строку не попадает вовсе.

**Чего маскирование не гарантирует.** Подмена работает по разбору текста запроса и по данным ответа, поэтому её границы принципиальные, а не настраиваемые.

- **Состав и порядок строк не закрываются.** Отбор по закрытому наименованию (`ГДЕ … ПОДОБНО`), сортировка по нему и `АВТОУПОРЯДОЧИВАНИЕ` выполняются: подмена меняет значения, но не то, какие строки вернулись и в каком порядке. Алфавитный порядок скрытых названий так восстановим. Это принятый остаточный риск — закрывать его было бы отказом, а отказов в контракте нет.
- **Числа и булево не закрываются.** Подмены, которая не искажает, для них нет, а `null` — стирание значения. Числовое поле в `type_field_masks` даёт предупреждение «маска не действует».
- **Плоская строка без ссылки** в отчётах (`run_1c_report`) и в произвольных выборках закрывается эвристикой по имени колонки: тип строки в данных не виден, обратного преобразования «строка → ссылка» не существует. Колонка с непохожим именем не опознаётся, и порогом сопоставления это не лечится.
- **Свободный текст** (`Комментарий`, `Примечание`) может содержать название — такие поля перечисляются в `fields` явно.
- **Одно юридическое лицо** часто есть и в `Справочник.Организации`, и в `Справочник.Контрагенты`: настраивать нужно оба справочника, иначе название уйдёт через второй.

Если поле или тип нужно закрыть так, чтобы значение не дошло до запроса вовсе, это делается allowlist'ом: `hidden_fields` у типа или `denied_objects` — там решение принимается до выполнения.
