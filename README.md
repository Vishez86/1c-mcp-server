...
Тип объекта определяется по данным ответа: сама ссылка (`type` + `uuid`), полное имя регистра в `register` или `accounting_register`, вложенная ссылка в `ref`, `target` или `item`, а если ключей контракта нет — любая колонка, значение которой является ссылкой. Последнее нужно для строк `run_1c_query`: там колонку со ссылкой называет автор запроса (`ВЫБРАТЬ К.Ссылка КАК Ссылка`), и без этого маски по типу к плоским колонкам строки не применялись бы. Если в строке несколько ссылок, контекст достаётся настроенному в политике типу, а не первому попавшемуся — иначе название незакрытого объекта маскировалось бы кодом чужого типа. Поля и строки внутри объекта наследуют его тип, а вложенная ссылка другого типа наследование сбрасывает.
