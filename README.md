...
Обе секции поддерживают необязательный `mode`: `"mask"` (по умолчанию) маскирует значения в ответах, `"deny"` дополнительно жёстко запрещает обращения к закрытым полям в запросах — предвалидатор выдаёт отказ `privacy_denied_field` до выполнения, в любой позиции текста (выбор, отбор, сортировка, итоги, функции, переименование `КАК X`); работа со ссылками остаётся разрешённой. Принадлежность поля разрешается по источникам запроса, поэтому одноимённое поле незакрытого типа (`Номенклатура.Наименование` рядом с закрытыми Контрагентами) не блокируется; разыменование любой глубины (`Поставщик.Наименование`), обращения без псевдонима и `ПРЕДСТАВЛЕНИЕ()`/`ПРЕДСТАВЛЕНИЕССЫЛКИ()` над закрытой ссылкой закрыты. Тип сегмента разрешается по всем коллекциям метаданных — реквизитам, стандартным реквизитам (`Ссылка`, `Владелец`, `Родитель`, `Регистратор`), измерениям и ресурсам регистров, субконто и счетам регистра бухгалтерии, — поэтому обходы через `К.Ссылка.Наименование`, `Д.Владелец.Наименование` и `ПРЕДСТАВЛЕНИЕ(О.СубконтоN)` тоже закрыты. Закрыты и три отдельных канала: разыменование после `ВЫРАЗИТЬ(… КАК <Тип>)` проверяется от типа приведения; разыменование колонки временной таблицы или подзапроса (`Т.Р.Наименование`) отклоняется, когда закрытый тип в запросе достижим, — простое чтение колонки-ссылки при этом разрешено; ссылка, переданная параметром (`{"kind":"ref","type":…}`), участвует в решении наравне с источниками, поэтому `ПРЕДСТАВЛЕНИЕ(&Реф)` и `&Реф.Наименование` по легально полученному uuid не отдают наименование. Отдельным кодом `privacy_denied_autoorder` отклоняется `АВТОУПОРЯДОЧИВАНИЕ` при достижимом закрытом типе: платформа сортирует ссылочные поля по представлению объекта, и порядок строк раскрывал бы алфавитный порядок скрытых наименований; обычная сортировка `УПОРЯДОЧИТЬ ПО` ссылке идёт по внутреннему представлению и разрешена. Имена функций сканируются как отдельные идентификаторы с допуском пробелов и переводов строки перед скобкой (`PRESENTATION (…)` не обходит проверку). Неразборный запрос с упоминанием закрытого типа отклоняется консервативно. Те же правила действуют на параметры остальных инструментов: поиск по наименованию в закрытом типе (`search_objects`, `item_query`), отборы/сортировки/группировки регистров и бухгалтерских инструментов, включая разыменование (`Контрагент.Наименование`), и параметры отчёта, совпавшие с закрытым полем; работа по ссылке и uuid (`find_object_by_id`, `item_ref`) остаётся разрешённой. Отказы пишутся в аудит без закрываемых значений. Если у закрытого типа есть подчинённые справочники, не указанные в политике, сервер предупреждает об этом в `config_warnings`. Проблемная запись в режиме `deny` (неизвестный `mode`, тип или поле не найдены в метаданных, плохой префикс, нечитаемая секция) переводит политику в `config_error`: запросы отклоняются с кодом `privacy_config_error` до исправления настройки (fail-closed), причины возвращаются в `privacy.config_errors`. `privacy.publish_details: false` убирает из guidance перечень закрытых полей, оставляя только типы.
