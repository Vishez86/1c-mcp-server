...
Сервер реализует протокол **Model Context Protocol (MCP) 2025-11-25** поверх HTTP-сервиса 1С и предоставляет LLM-агентам 38 read-only инструментов согласно спецификации `mcp_1c_tools_spec.md`.
...
- 38 tools: discovery → inspect → search → retrieve → explain → navigate → report → query guidance → generated language docs → data passport → query examples → legal sources → тайминги вызовов.
...
| 38 | `get_audit_log` | Тайминги вызовов MCP из журнала регистрации |
| 39 | `get_database_passport_full` | Перепись метаданных счётчиками по видам (без имён объектов) |
...
Сервер умеет отдавать данные LLM с подменой чувствительных значений: запрос выполняется, строки возвращаются, но значения закрытых полей заменены — строки на `XXXXXXX`, даты на `1900-01-01T00:00:00`. Числа и булево не закрываются: подмены, которая не искажает, для них нет. Ссылка сохраняет `type`, `uuid` и `navigation_url`, поэтому легальный пользователь открывает объект в 1С и видит настоящее значение.

**Состав маскирования выбирает администратор**, а не код. Никаких автоматических расширений «раз закрыли фамилию — закроем и место рождения» нет: закрыто ровно то, что помечено.

### Где что настраивается

| Что | Где хранится | Чем меняется |
|---|---|---|
| Состав: типы и их поля, псевдонимы | справочник `MCP_Маскирование`, предопределённый элемент «Настройки маскирования» (JSON в реквизите `Значение`) | обработка `MCP_MaskingRule` |
| Рубильник «применять или нет» | константа `MCP_ServerConfig`, ключ `privacy.enabled` | кнопка в той же обработке |

Разнесены намеренно: испорченный JSON состава уводит сервер в аварийный режим с максимальной маской, и выключатель не должен зависеть от испорченного артефакта. Отсутствие ключа, секции или константы трактуется как «включено» — выключает только явная `false`.

📖 **Как этим пользоваться — `doc/masking_admin_guide.md`**: зачем нужно, что закрывается, работа с обработкой, разбор отчёта, проверка результата снаружи, типичные ошибки.
...
```json
{
  "privacy": {
    ...
    "type_field_masks": [
      { "type": "Справочник.ФизическиеЛица", "fields": ["Наименование", "ИНН"] }
    ],
...
- `masked_fields` — поля, закрываемые у **всех** объектов сразу. Сравнение имён регистронезависимое, не учитывает пробелы, дефисы и подчёркивания, поэтому правило не привязано к конкретной конфигурации.
- `type_field_masks` — поля, закрываемые **только** у указанного типа: `Наименование` у одного справочника можно закрыть, оставив у другого открытым. Вид типа любой, включая регистры.
- `type_aliases` — вместо одинакового `XXXXXXX` подставляется стабильный 8-символьный код от uuid ссылки (`Контр-85KYOVOH`), одинаковый для объекта в разных ответах: обезличенные строки остаются сопоставимыми. Доступно только ссылочным типам; регистры и перечисления отклоняются с причиной в `privacy.config_warnings`.
- `publish_details` — публиковать ли в подсказке для модели перечень закрытых полей.

### Класс имени объекта

Пометка любого поля роли «имя» — `Наименование`, `НаименованиеПолное`, `КраткоеНаименование`, `ФИО` — закрывает весь класс сразу: саму колонку, `ПРЕДСТАВЛЕНИЕ(Ссылка)` и `presentation` внутри структуры ссылки. Иначе название утекало бы через любую ссылку из соседнего объекта.

`Код` в класс **не входит** и остаётся открытым намеренно: он не имя, а стабильный идентификатор, и через него обезличенные строки различимы между собой.

Имена, специфичные для конфигурации (`НаименованиеСлужебное`, `Инициалы`, `НаименованиеБанка`, `НаименованиеОКВЭД`), в канонический словарь не входят: эвристика «начинается на Наименование» закрыла бы последние два, которые персональными данными не являются. Такие поля закрываются только явной пометкой.

### Границы механизма

- **Технические имена метаданных не маскируются**: `type`, `full_name`, `kind`, `register`, `chart`, `resource_uri` остаются валидными идентификаторами 1С. Privacy применяется к данным и представлениям ссылок, а не к схеме.
- **Плоская строка без ссылки** — частый случай, когда рядом с субконто идёт колонка с названием. Стабильный код построить не из чего, поэтому подставляется `<Префикс>скрыто` либо `XXXXXXX`. Опознание идёт по имени колонки и является эвристикой: колонка с непохожим именем или произвольным псевдонимом (`КАК Н`) не распознаётся.
- **Одно юридическое лицо** часто присутствует и в `Справочник.Организации`, и в `Справочник.Контрагенты` — настраивать нужно оба, иначе название уйдёт через второй.
- Если поле или тип нужно закрыть так, чтобы значение не дошло до запроса вовсе, это делается allowlist'ом (`hidden_fields`, `denied_objects`): там решение принимается до выполнения.

### Что видит клиент

Когда защита действует, сервер сообщает об этом явно: блок `privacy` в каждом tool-result и в `get_current_user_context`, плюс короткая подсказка в `tools/list`. Форма блока: `{ "enabled": true, "masked_fields": [...], "string_mask": "XXXXXXX", "date_mask": "1900-01-01T00:00:00", "type_aliases": { "enabled": true, "entries": [...] }, "type_field_masks": { "enabled": true, "entries": [...] }, "config_warnings": [], "config_errors": [], "engine_revision": "…", "guidance": "…" }`.

**`enabled: false` не означает «рубильник выключен»** — это эффективный признак «маскирование не действует», и он ложен и при выключенном рубильнике, и при пустом составе. Различать по тексту `guidance`: упоминание `privacy.enabled в MCP_ServerConfig` — выключен рубильник; «Privacy-маскирование отключено; значения полей возвращаются согласно allowlist» — рубильник включён, применять нечего.

Агент должен считать закрытые поля намеренно недоступными: подмена сделана по разбору текста запроса, поэтому обходная формулировка не столько раскрывает данные, сколько портит ответ — вместо названия придёт маска.
