...
> `list_legal_sources` / `get_legal_source_guide` — реестр разрешённых правовых источников и инструкции по работе с ними. Встроенные источники — два раздела pravo.gov.ru: `pravo_gov_ru` («Официальное опубликование», открытый API, первоначальные редакции) и `pravo_gov_ru_actual` («Тексты правовых актов с внесёнными изменениями», actual.pravo.gov.ru — актуальные консолидированные редакции кодексов и федеральных актов, включая будущие редакции). Политика для LLM-агентов: законодательство запрещено искать в открытом интернете — только через источники этого реестра по их инструкциям. Реестр зашит в модуль `MCP_LegalSources`; при наличии в конфигурации регистра сведений `MCP_ПравовыеИсточники` его записи дополняют и переопределяют встроенные без обновления расширения (см. `INSTALL.md`).
...
Тип объекта определяется по данным ответа: сама ссылка (`type` + `uuid`), полное имя регистра в `register` или `accounting_register`, вложенная ссылка в `ref`, `target` или `item`. Поля и строки внутри такого объекта наследуют его тип, а вложенная ссылка другого типа наследование сбрасывает. Благодаря этому маски по типу работают и для строк `get_register_records`/`get_document_movements` (ключ `register`), и для бухгалтерских инструментов (`accounting_register`), и для `get_object_history` (`target`).
