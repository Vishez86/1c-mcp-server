...
**Ограничения:** ориентирован на бухгалтерские регистры и типовые субконто номенклатуры/складов. Если в базе другой план счетов или названия видов субконто, передайте их явно.
...
**Ограничения:** `base_url` должен быть разрешён в allowlist. `thin_client` в универсальной реализации может вернуть предупреждение о неподдержанном формате.
...
`organization_aliases` по умолчанию выключен. Если включить `enabled: true`, сервер заменяет полные названия `Справочник.Организации` на стабильные коды вида `Орг-453276` во всех tool-ответах, ресурсах и диагностических JSON-данных. Ссылки сохраняют `type`, `uuid`, `ref` и доступный `navigation_url`; пользователь открывает `navigation_url` в 1С, чтобы увидеть реальное название по своим правам. Если в произвольном результате есть только строковое поле, явно похожее на организацию, но нет UUID/ссылки, сервер возвращает `Орг-скрыто`.

`person_aliases` по умолчанию выключен. Если включить `enabled: true`, сервер заменяет `presentation` и поля ФИО у `Справочник.ФизическиеЛица` и `Справочник.Сотрудники*` стабильными кодами вида `ФЛ-453276` или `Сотр-453276`. Паспортные данные, индивидуальный номер, страховой номер и дата рождения маскируются как чувствительные поля. Если в произвольном результате есть только строковое поле, явно похожее на физлицо/сотрудника, но нет UUID/ссылки, сервер возвращает `ФЛ-скрыто`.
